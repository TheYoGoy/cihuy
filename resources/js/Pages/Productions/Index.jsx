...
import { Search, Plus, Edit3, Trash2, Coffee, AlertCircle, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function ProductionIndex() {
    ...
    useEffect(() => {
        ...
        console.log('Productions.data is array:', Array.isArray(productions?.data));
    ...
    const handleExportPdf = () => {
        ...
        const url = route("productions.export.pdf") + 
                    (params.toString() ? '?' + params.toString() : '');
        window.open(url, "_blank");
    };
    ...
    const isPaginated = productions && typeof productions === 'object' && !Array.isArray(productions) && productions.links;

    // Pagination Component
    const PaginationComponent = () => {
        if (!isPaginated || !productions.links || productions.links.length <= 3) {
            return null;
        }
        ...
        return (
            <div className="flex items-center justify-between mt-6 px-4 py-3 bg-cyan-600 border rounded-lg shadow-sm ">
                <div className="flex items-center text-sm ">
                    <span>
                        Menampilkan {productions.from || 1} sampai {productions.to || productionData.length} dari {productions.total || productionData.length} data
                    </span>
                </div>
        ...
        >
                        ...
                        <Card className="shadow-xl bg-cyan-600 border-0 overflow-hidden">
                            <CardHeader className="pb-6 bg-cyan-700 border-b border-cyan-500">
                                ...
                                <p className="text-cyan-100 text-sm mt-2 font-medium">
                                    Masukkan detail produksi untuk produk yang dipilih.
                                </p>
                            </CardHeader>

                            <CardContent className="pt-6 px-6 pb-6">
                                <form onSubmit={handleAddProduction} className="space-y-6">
                                        ...
                                        >
                                            {processing ? (
                                                ...
                                                <div className="flex items-center justify-center group">
                                                    ...
                                                    <span className="font-semibold">Tambah Produksi</span>
                                                </div>
                                            )}
                                        </Button>
                                    </div>
                            ...
                            <CardHeader className="pb-4">
                                <CardTitle className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                    ...
                                    <div className="bg-cyan-100 text-cyan-800 px-3 py-1 rounded-full text-sm font-semibold">
                                        Total: {isPaginated ? productions.total : productionData.length} data
                                    </div>
                                </CardTitle>
                                ...
                                {!Array.isArray(productionData) || productionData.length === 0 ? (
                                    <div className="text-center py-16">
                                        ...
                                        <h3 className="text-xl font-semibold text-gray-600 mb-2">
                                            {search ? `Tidak ada hasil untuk "${search}"` : "Belum ada catatan produksi"}
                                        </h3>
                                        <p className="text-gray-500 text-lg">
                                            {search ? "Coba kata kunci lain atau hapus filter pencarian." : "Tambahkan catatan produksi untuk memulai."}
                                        </p>
                                        {search && (
                                            <Button
                                                onClick={handleClearSearch}
                                        ...
                                        <div className="mt-4 text-xs text-gray-400">
                                            Debug: productionData type = {typeof productionData}, 
                                            isArray = {Array.isArray(productionData).toString()},
                                            length = {productionData?.length || 'undefined'}
                                        </div>
                                    </div>
                                ) : (
                                    <>
                                                        ...
                                                        >
                                                            <TableCell className="text-center">
                                                                <div className="bg-amber-100 text-amber-800 text-xs font-bold px-2 py-1 rounded-full w-8 h-8 flex items-center justify-center mx-auto">
                                                                    {isPaginated ? 
                                                                        ((productions.current_page - 1) * productions.per_page) + index + 1 
                                                                        : index + 1
                                                                    }
                                                                </div>
                                                            </TableCell>
                                                            ...
                                                            <TableCell className="text-gray-600">
                                                                <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-sm font-medium">
                                                                    {Number(production.quantity).toLocaleString()} unit
                                                                </span>
                                                            </TableCell>
                ...
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-gray-600 mt-3">
                            Apakah Anda yakin ingin menghapus catatan produksi untuk produk{" "}
                            ...
                            <span className="font-semibold ">
                                {productionToDelete?.production_date ? 
                                    new Date(productionToDelete.production_date).toLocaleDateString("id-ID") 
                                    : ''
                                }
                            </span>
            ...
            <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
                <DialogContent className="sm:max-w-2xl">
                    ...
                    <form onSubmit={handleUpdateProduction} className="space-y-4 mt-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {/* Produk */}
                            <div>
                                <Label>Produk</Label>
                                <Select
                                    value={editData.product_id?.toString() || ""}
                                    onValueChange={(value) => setEditData("product_id", parseInt(value))}
                                >
                            ...
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setEditData("production_date", e.target.value)}
                                    className="mt-1"
                                />
